...
public class Bat extends Enemy
{
	...
	// Create Vector2 variable for Bat Velocity
	...
	// Boolean variable to determine if we need to destroy Bat box2DBody outside of the simulation
	...
	public Bat(PLordPlayScreen screen, float x, float y) 
	{
		super(screen, x, y);
		...
		for (int i = 1; i < 3; i++)
			frames.add(new TextureRegion(screen.getAtlas().findRegion("Bat"), i*35, 0 * 37, 35, 35));
		...
		// Set size and initial position of Bat Sprite, 27X27 seems like good size for Bat (both have to be divided by our Pixels_per_meter 1st)
		setBounds(getX(), getY(), 27 / PhantomLord.PIXELS_PER_METER, 27 / PhantomLord.PIXELS_PER_METER);
		
		// Define bats velocity to be somehting other than default velocity for "Enemy class"
		// Define Enemy Velocity (1 in x axis, -1.7 in y axis, so they can fall if need be)
		velocity = new Vector2(0f, .5f);
	
		// Assume initially that Bat is not destroyed and not about to be destroyed
		setToDestroy = false;
		destroyed = false;
	...
	public void update(float dt, PLordPlayScreen screen)
	{
		...
		// Makes it so we don't make the enemy start moving depending on if they are close to screen or not, this si because
		// bat changes its velocity depending on the current time, so if we did that, would never know exactly what y velcoity
		// bat would move in, making it difficult to place it
		...
		// box2DBody
		// Note: I added "+ (2 / PhantomLord.PIXELS_PER_METER)" to vertical position cause Bat seem slightly sunk into the ground, so I moved him up by 4 pixels relative to his box2DBody
        ...
        if(nonStompKill)
        {
			...
			// Deterime if we have already started counting the time before removing Bat sprite (need it to show for a second for disappearing)
			if (waitToRemoveSprite == false)
			{
				waitToRemoveSprite = true;
				
				//Reset timer to know how long the Bat has been dead
				stateTimer = 0;
			}
    		...
    		{
				// Remove the Bat box2DBody (still need to override draw mehtod to remove sprite, method "draw" is in this class)
				if(box2DBody != null)
				...
				Box2DWorldCreator.removeBat(this);
		...
		// Need to check if Bat is being setToDestroy, if it is destroy box2DBody for Bat and show the smashed Bat sprite
        else if(setToDestroy && !destroyed)
		{
        	// Makes it so Bat doesn't dive off edge of screen when jumpin on them
			...
			// Need to set Sprite visual texture to "Smashed Bat" (since it was now hit)
			// Use the Region for "Bat" find the sprite frame and enter its x & y coordinates, and its width and height
			// Can find it in the "Bat.png" from "AllSpriteSheets (For libGDX Texture Packer)" and use MediBangPaint to find coordinate if need be
			...
			setRegion(new TextureRegion(screen.getAtlas().findRegion("Bat"), 0*35, 0*35, 35, 35));
			...
			{
				// Remove the Bat box2DBody (need to override draw mehtod to remove sprite, method "draw" is in this class)
				if(box2DBody != null)
					world.destroyBody(box2DBody);
        ...
        // In TileD, bat will go up 6 "tiles" from where he was positioned in titled, and then back down 6 tiles to where he originally was positioned
        else if(stateTimer > 2)
		{
			// Reverse velocity in y axis
			reverseVelocity(false, true);
			
			stateTimer = 0;
		}
		
		// play bounding Bat animation and set position of sprite relative to box2DBody
		else if(!destroyed)
		{
			// Set linear velocity (movement) if Bat not destroyed, set the velocity to the velocity Vector2 object from the 'enemy' class
			box2DBody.setLinearVelocity(velocity);
			...
			if(flyAnimation.getKeyFrame(stateTimer, true).isFlipX() == false)
			{
				// Only flipping HatGhost on x axis
				flyAnimation.getKeyFrame(stateTimer, true).flip(true, false);
			}
	...
	*/
	...
	{
		// Define body for Bat and assign body to world
		BodyDef bDef = new BodyDef();
		bDef.position.set(getX(), getY());																// Box2D Body is at position of created Bat Sprite, after this the
		bDef.type = BodyDef.BodyType.DynamicBody;														// sprite/animation stays in line with the actual physics collider Also, 
		box2DBody = world.createBody(bDef);																// when we Bat moves, the box2DBody gets moved to correspond to that,
		box2DBody.setGravityScale(0);																								// so the Spite needs to be rendered wherever the box2DBody is
		
		// Define fixture for Bat (make radius appropriate size that matches the Bat sprite)
		FixtureDef fDef = new FixtureDef();
		CircleShape shape = new CircleShape();
		shape.setRadius(6 / PhantomLord.PIXELS_PER_METER);
		
		// Category for Bat, aka, what is this fixture's category
		...
		// Set this fixture to our body for Bat and setUserData for WorldContactLister
		fDef.shape = shape;
		box2DBody.createFixture(fDef).setUserData(this);
		
		// Create the Head for Bat
		...
		fDef.shape = head;
		// Add Bounciness, so when PLord hits head o Bat, he bounces up a bit
		// This is done by setting the restitution of the fixture
		fDef.restitution = 3.5f;			// Full bounciness, so if was 10 pixels from top of head, then hit, PLord will bounce 5 pixels off
		...
		// Fix issue where Bat will respond to any collision of the ground beneath it, only care about the ground that
		// relate to walls, so create 'Feet' for Bat so it doesn't ever touch the bottom part of ground
		...
		// Coordinates relative to Bat body, edgeShape acts as feet to glide across tiles (since radius of Bat is 6, go -7 below the
		// center, and the 'feet' will be exactly 1 pixels below the body of Bat, which seems good
	...
	{
		// if Bat is not destroyed or the stateTimer is less than 1 second
	...
	{
		// If gets hit by enemmy, want to check if Bat gets hit by HatGhost, and if so, is it in MOVING HIDEINHAT state?
		// if so that hat should hit and kill Bat
		if(enemy instanceof HatGhost && ((HatGhost) enemy).currentState == HatGhost.State.MOVING_HIDEINHAT)
		{
			...
			// Add score to HUD, as PLord managed to hit a GhostHat on head, and have hat kill Bat
	...
	{
		// logic for what happens when Bat gets hit on the head by PLord
		
		// Add score to HUD, pLOrd killed Bat Enemy
